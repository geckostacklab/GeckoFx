...
import IsometricCameraCode from '../../components/svg/IsometricCamera?raw';
import Button from '../../components/ui/layout/Button';
import DocsTab from '../../components/ui/layout/DocsTab';
import { useState, useRef, useEffect } from 'react';
import CodeTab from '../../components/ui/layout/CodeTab';

export const Route = createLazyFileRoute('/docs/isometric-camera')({
    ...
    <div ref={containerRef} className='h-fit w-full p-4 lg:p-8 flex flex-col gap-6 lg:gap-8'>
      <div className="flex flex-wrap justify-between gap-2">
        <div className='flex items-center'>
          ...
          <h1 className='text-xl font-bold text-primary'>{"IsometricCamera"}</h1>
      ...
      {tab === "docs" ? (
        ...
        <CodeTab source={IsometricCameraCode} width={containerWidth} />
      )}
    </div>
  </div>
}
