...
import FastZap from '../../components/svg/FastZap';
import FastZapCode from '../../components/svg/FastZap?raw';
import Button from '../../components/ui/layout/Button';
import DocsTab from '../../components/ui/layout/DocsTab';
import { useState, useRef, useEffect } from 'react';
import CodeTab from '../../components/ui/layout/CodeTab';

export const Route = createLazyFileRoute('/docs/fast-zap')({
    ...
    <div ref={containerRef} className='h-fit w-full p-8 flex flex-col gap-8'>
      <div className="flex justify-between">
        <div className='flex items-center'>
          ...
          <h1 className='text-xl font-bold text-primary'>{FastZap.name}</h1>
      ...
      {tab === "docs" ? (
        ...
        <CodeTab source={FastZapCode} width={containerWidth} />
      )}
    </div>
  </div>
}
